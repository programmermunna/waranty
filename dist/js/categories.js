...
const renderCategories = (get_categories) => {
    ...
    get_categories?.map((c, i) => {
       ...
       <div class="flex items-center justify-between hover:bg-gray-100">
        <div style="padding-left:${6}px; white-space:nowrap;" >
          ${c?.children?.length > 0
...
const all_parent_cat = document.querySelectorAll("#parent_cat");
const all_categories_ul_ref_parent = document.querySelectorAll(
...
if (hide_categories_ul_ref_parent) {

    hide_categories_ul_ref_parent.addEventListener("click", function () {
        all_categories_ul_ref_parent.forEach((ele) => {
...
if (all_parent_cat?.length > 0) {
    all_parent_cat.forEach((parent_cat) => {
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    all_select_ref_cat.forEach((btn, i) => {
        btn.addEventListener("click", function () {
            all_parent_cat.forEach((parent_cat) => {
...
const all_add_category_wrapper = document.querySelectorAll(".add_category_wrapper");

all_show_add_new_cat.forEach((btn) => {
    btn.addEventListener("click", function () {
        all_add_category_wrapper.forEach((add_category_wrapper) => {
